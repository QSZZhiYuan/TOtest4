...
public class PhantomRageOverlay {
    ...
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        if (USE_XP_BAR) {
            ...
            guiGraphics.renderComponentHoverEffect(PHANTOM_RAGE_TEXTURE, x, y, 29.0f, 89.0f, 182, 5, 241, 115);
            int fillWidth = (int)(182.0 * progress);
            if (fillWidth > 0) {
                guiGraphics.renderComponentHoverEffect(PHANTOM_RAGE_TEXTURE, x, y, 29.0f, 99.0f, fillWidth, 5, 241, 115);
            }
            if ((cooldownFillWidth = (int)(182.0 * cooldownProgress)) > 0) {
                guiGraphics.renderComponentHoverEffect(PHANTOM_RAGE_TEXTURE, x, y, 29.0f, 78.0f, cooldownFillWidth, 5, 241, 115);
            }
            poseStack.popPose();
        } else {
            int cooldownFillWidth;
            ...
            guiGraphics.renderComponentHoverEffect(PHANTOM_RAGE_TEXTURE, 0, 0, 55.0f, 28.0f, 130, 21, 241, 115);
            int fillWidth = (int)(130.0 * progress);
            if (fillWidth > 0) {
                guiGraphics.renderComponentHoverEffect(PHANTOM_RAGE_TEXTURE, 0, 0, 55.0f, 51.0f, fillWidth, 21, 241, 115);
            }
            if ((cooldownFillWidth = (int)(130.0 * cooldownProgress)) > 0) {
                guiGraphics.renderComponentHoverEffect(PHANTOM_RAGE_TEXTURE, 0, 0, 55.0f, 6.0f, cooldownFillWidth, 21, 241, 115);
            }
            poseStack.popPose();
        }
    }
    ...
    public static void onRenderXpBar(RenderGuiOverlayEvent.Pre event) {
        ...
        ItemStack chestplate = player.shouldRemoveSoulSpeed(EquipmentSlot.CHEST);
        if (chestplate.onUseTick() || chestplate.setRepairCost() != TravelopticsItems.CURSED_WRAITHGUARD_ARMOR_CHESTPLATE.get()) {
            return;
        }
        if (USE_XP_BAR) {
            if (!shouldRenderRageBar) {
