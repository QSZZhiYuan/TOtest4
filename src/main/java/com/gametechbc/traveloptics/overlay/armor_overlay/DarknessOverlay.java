...
public class DarknessOverlay {
    private static final ResourceLocation DARKNESS_TEXTURE = new ResourceLocation("traveloptics", "textures/gui/armor/darkness_bar.png");
    ...
    private static final int BAR_X_OFFSET = (Integer)ClientConfig.darknessXOffset.get();
    private static final int BAR_Y_OFFSET = 90 + (Integer)ClientConfig.darknessYOffset.get();
    ...
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        if (!TOArmorUtils.isWearingFullSet(player, ForlornHarbingerArmorItem.class)) {
            return;
        }
        int currentFuel = DarknessManager.getDarkness(chestplate);
        int maxFuel = 300;
        ...
        if (USE_XP_BAR) {
            ...
            guiGraphics.renderComponentHoverEffect(DARKNESS_TEXTURE, x, y, 29.0f, 89.0f, 182, 5, 241, 115);
            int fillWidth = (int)(182.0 * progress);
            if (fillWidth > 0) {
                guiGraphics.renderComponentHoverEffect(DARKNESS_TEXTURE, x, y, 29.0f, 99.0f, fillWidth, 5, 241, 115);
            }
            poseStack.popPose();
        } else {
            int barWidth = 134;
            ...
            guiGraphics.renderComponentHoverEffect(DARKNESS_TEXTURE, 0, 0, 53.0f, 24.0f, 134, 21, 241, 115);
            int fillWidth = (int)(134.0 * progress);
            if (fillWidth > 0) {
                guiGraphics.renderComponentHoverEffect(DARKNESS_TEXTURE, 0, 0, 53.0f, 55.0f, fillWidth, 21, 241, 115);
            }
            poseStack.popPose();
        }
    }
    ...
    public static void onRenderXpBar(RenderGuiOverlayEvent.Pre event) {
        ...
        ItemStack chestplate = player.shouldRemoveSoulSpeed(EquipmentSlot.CHEST);
        if (chestplate.onUseTick() || chestplate.setRepairCost() != TravelopticsItems.FORLORN_HARBINGER_ARMOR_ROBE.get()) {
            return;
        }
        if (USE_XP_BAR) {
            if (!shouldRenderFuelBar) {
