...
public class PrimordialCrestOverlay {
    ...
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        if (USE_XP_BAR) {
            ...
            guiGraphics.renderComponentHoverEffect(PRIMORDIAL_CREST_TEXTURE, x, y, 29.0f, 89.0f, 182, 5, 241, 115);
            int fillWidth = (int)(182.0 * (1.0 - progress));
            if (fillWidth > 0) {
                guiGraphics.renderComponentHoverEffect(PRIMORDIAL_CREST_TEXTURE, x, y, 29.0f, 99.0f, fillWidth, 5, 241, 115);
            }
            poseStack.popPose();
        } else {
            int barWidth = 135;
            ...
            guiGraphics.renderComponentHoverEffect(PRIMORDIAL_CREST_TEXTURE, 0, 0, 53.0f, 20.0f, 135, 25, 241, 115);
            int fillWidth = (int)(135.0 * (1.0 - progress));
            if (fillWidth > 0) {
                guiGraphics.renderComponentHoverEffect(PRIMORDIAL_CREST_TEXTURE, 0, 0, 53.0f, 48.0f, fillWidth, 25, 241, 115);
            }
            poseStack.popPose();
        }
    }
    ...
    public static void onRenderXpBar(RenderGuiOverlayEvent.Pre event) {
        ...
        ItemStack chestplate = player.shouldRemoveSoulSpeed(EquipmentSlot.CHEST);
        if (chestplate.onUseTick() || chestplate.setRepairCost() != TravelopticsItems.PRIMORDIAL_CREST_ARMOR_CHESTPLATE.get()) {
            return;
        }
        if (USE_XP_BAR) {
            if (!shouldRenderFuelBar) {
