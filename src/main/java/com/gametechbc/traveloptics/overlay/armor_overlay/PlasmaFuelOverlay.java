...
public class PlasmaFuelOverlay {
    ...
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        if (USE_XP_BAR) {
            ...
            guiGraphics.renderComponentHoverEffect(PLASMA_FUEL_TEXTURE, x, y, 29.0f, 89.0f, 182, 5, 241, 115);
            int fillWidth = (int)(182.0 * progress);
            if (fillWidth > 0) {
                guiGraphics.renderComponentHoverEffect(PLASMA_FUEL_TEXTURE, x, y, 29.0f, 99.0f, fillWidth, 5, 241, 115);
            }
            poseStack.popPose();
        } else {
            int barWidth = 124;
            ...
            guiGraphics.renderComponentHoverEffect(PLASMA_FUEL_TEXTURE, 0, 0, 100.0f, 40.0f, 124, 11, 241, 115);
            int fillWidth = (int)(124.0 * progress);
            if (fillWidth > 0) {
                guiGraphics.renderComponentHoverEffect(PLASMA_FUEL_TEXTURE, 0, 0, 100.0f, 55.0f, fillWidth, 11, 241, 115);
            }
            poseStack.popPose();
        }
    }
    ...
    public static void onRenderXpBar(RenderGuiOverlayEvent.Pre event) {
        ...
        ItemStack chestplate = player.shouldRemoveSoulSpeed(EquipmentSlot.CHEST);
        if (chestplate.onUseTick() || chestplate.setRepairCost() != TravelopticsItems.MECHANIZED_EXOSKELETON_ARMOR_CHESTPLATE.get()) {
            return;
        }
        if (USE_XP_BAR) {
            if (!shouldRenderFuelBar) {
